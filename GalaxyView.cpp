...
// Since QSlider only accepts ints we will divide all of the values by this in order to get a float
const double SLIDER_FLOAT_SCALE = 100;
const double PAINT_TOOL_RANGE_MAX = 512;

void GalaxyView::SetToolOptions()
{
    QFrame * popup = new QFrame(this, Qt::Window);
    QVBoxLayout* layout = new QVBoxLayout;
    popup->setLayout(layout);
    popup->setGeometry(0,0,100,100);

    double * option_pointers[] = {
        &paintToolRange,
        &paintToolSeparationAverage,
        &paintToolSeparationDeviation,
        &connectionDistanceAverage,
        &connectionDistanceDeviation,
        &inhabitedFraction,
        &spaceportFraction,
    };
    std::string option_names[] = {
        "Range of the paint tool",
        "Average separation of stars generated by the paint tool",
        "Standard deviation of the separation",
        "Average maximum distance required for connecting stars",
        "Standard deviation of the maximun distance",
        "Fraction of the systems that have a landable inhabited planet",
        "Fraction of the inhabited planets that have a spaceport",
    };
    double option_min[] = {0,1,0,1,0,0,0};
    double option_max[] = {PAINT_TOOL_RANGE_MAX,PAINT_TOOL_RANGE_MAX, paintToolSeparationAverage, PAINT_TOOL_RANGE_MAX, connectionDistanceAverage, 1,1};

    for (long unsigned int i = 0; i < sizeof(option_pointers) / sizeof(double*); ++i) {

        auto text_label = new QLabel(QString::fromStdString(option_names[i]), this);
        auto value_label = new QLabel(QString::fromStdString(to_string(*option_pointers[i])), this);
        layout->addWidget(text_label);
        layout->addWidget(value_label);
        auto slider = new QSlider(popup);
        slider->setOrientation(Qt::Horizontal);
        slider->setMinimum(option_min[i] * SLIDER_FLOAT_SCALE);
        slider->setMaximum(option_max[i] * SLIDER_FLOAT_SCALE);
        slider->setSingleStep((double)(option_max[i] - option_min[i]) * SLIDER_FLOAT_SCALE);
        slider->setValue(*option_pointers[i] * SLIDER_FLOAT_SCALE);

        layout->addWidget(slider);
        auto on_value_changed = [this, option_pointers, i, slider, value_label]() {
            *option_pointers[i] = (double)slider->value() / SLIDER_FLOAT_SCALE;
            value_label->setText(QString::fromStdString(to_string(*option_pointers[i])));
        };
        connect(slider, &QSlider::valueChanged, this, on_value_changed);
    }
    popup->show();
}
...
void GalaxyView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::RightButton)
    {
        ...
        std::normal_distribution<double> separation(paintToolSeparationAverage,paintToolSeparationDeviation);
        std::normal_distribution<double> connection(connectionDistanceAverage,connectionDistanceDeviation);
        QVector2D origin = MapPoint(event->pos());
        int start_x = origin.x() - paintToolRange / 2;
        int start_y = origin.y() - paintToolRange / 2;

        System * previous = nullptr;

        if(systemView && systemView->Selected())
        {
            previous = systemView->Selected();
        }

        // Tile the paint area in quadrants
        int last_separation_x = separation(rng);
        for (int x =  start_x; x < start_x + paintToolRange; x+=last_separation_x)
        {
            int last_separation_y = separation(rng);
            for (int y = start_y; y < start_y + paintToolRange; y+=last_separation_y)
            {
                float ox = (float)rand()/(float)(RAND_MAX) * last_separation_x;
                float oy = (float)rand()/(float)(RAND_MAX) * last_separation_y;
                QVector2D pos = QVector2D(ox+x, oy+y);

                // Don't place stars outside the circle
                if((origin - pos).length() > (paintToolRange / 2)) {
                    continue;
                }
                ...
                for(auto &it : mapData.Systems())
                {
                    if(pos.distanceToPoint(it.second.Position()) < paintToolSeparationAverage)
                    {
                        doNotCreate = true;
                        break;
                    }
                ...
                QString text = QString::fromStdString(std::to_string(mapData.Systems().size()));
                text = QString("AUTOSYSTEM_") + text;
                System &system = mapData.Systems()[text];
                system.Init(text, pos);
                bool inhabited = (rand() / RAND_MAX) < inhabitedFraction;
                system.Randomize(inhabited, inhabited);

                // Copy stats from selected system
                if (previous) {
                ...
                if(inhabited) {
                    for(StellarObject& object : system.Objects()) {
                        if (object.IsPotentiallyHabitable(system.HabitableZone())) {
                            QString text = QString::fromStdString(std::to_string(mapData.Planets().size()));
                            object.SetPlanet("AUTOPLANET_" + text);

                            Planet &planet= mapData.Planets()["AUTOPLANET_" + text];
                            planet.SetName("AUTOPLANET_" + text);
                            planet.SetDescription("AUTOPLANET_" + text + "_DESC");

                            if ((rand() / RAND_MAX) < spaceportFraction) {
                                planet.SetSpaceportDescription("AUTOPLANET_" + text + "_SPACEPORT");
                            }
                ...
                last_separation_y = separation(rng);
            }
            last_separation_x = separation(rng);
        }

        for (auto &from: mapData.Systems()) {
            // Don't connect to systems outside of the tool
            if ((from.second.Position() - origin).length() > (paintToolRange / 2)){
                continue;
            }
            for(auto &to : mapData.Systems())
            {
                // Don't connect to systems outside of the tool
                if ((to.second.Position() - origin).length() > (paintToolRange / 2)){
                    continue;
                }
                if(from.second.Position().distanceToPoint(to.second.Position()) < connection(rng))
                {
                    // If not linked yet, add it
                    if (from.second.Links().find(to.first) == from.second.Links().end())
                    {
                        from.second.ToggleLink(&to.second);
                    }
                }
            }
        }
...
void GalaxyView::wheelEvent(QWheelEvent *event)
{
    ...
    // This part of the code looks like Python
    if(event->modifiers() & Qt::ShiftModifier)
        if (event->delta() > 0)
            if (paintToolRange)
                paintToolRange = min(paintToolRange*1.3, PAINT_TOOL_RANGE_MAX);
            else
                paintToolRange = 10;
        else if(paintToolRange < 10)
            paintToolRange = 0;
        else
            paintToolRange /= 1.3;
    else
...
void GalaxyView::paintEvent(QPaintEvent */*event*/)
{
    ...
    painter.setPen(QPen(QBrush(QColor(255,255,255), Qt::BrushStyle::SolidPattern),1/scale));
    painter.drawEllipse(MapPoint(mapFromGlobal(QCursor::pos())).toPointF(), (int)(paintToolRange), (int)paintToolRange);
}
